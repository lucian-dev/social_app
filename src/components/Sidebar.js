...
const Sidebar = ({ user, closeToggle }) => {
  ...
  return (
    <div className="flex flex-col justify-between bg-white h-full overflow-y-scroll min-w-210 hide-scrollbar">
        ...
        <div className="flex flex-col gap-5">
          ...
          {categories.slice(0, categories.length - 1).map((category) => (
            <NavLink to={`/category/${category.name}`} className={({ isActive }) => (isActive ? isActiveStyle : isNotActiveStyle)} onClick={handleCloseSidebar} key={category.name}>
              <img src={category.image} alt="cat-img" className="w-8 h-8 rounded-full shadow-sm object-cover" />
              {category.name}
            </NavLink>
          ))}
        </div>
      ...
      {user && (
        <Link to={`/user-profile/${user._id}`} className="flex items-center my-5 mb-3 gap-2 p-2 bg-white rounded-lg shadow-lg mx-3" onClick={handleCloseSidebar}>
          <img src={user.image ? user.image : `${defaultThumb}`} className="w-10 h-10 rounded-full" alt={user.profile} />
          <p>{user.userName}</p>
        </Link>
      )}
    </div>
